...
def testapi_dashboard(request):
    ...
    for test in final_tests:
        # Use semester saved on the Test if available
        semester_id = getattr(test, 'semester_id', None)
        # Legacy student test yozuvlari group_id ga bog'langan; agar test.group yo'q va M2M orqali keldi
        # bo'lsa, group_id sifatida studentning guruhini qo'llaymiz.
        effective_group_id = test.group_id or getattr(request.user.group, 'id', None)
        filter_kwargs = dict(
...
def testapi_test(request, test_id):
    ...
    # Legacy single group; agar yo'q bo'lsa M2M dan talabaning guruhini ishlatamiz
    group = test.group or (request.user.group if test.groups.filter(id=getattr(request.user.group, 'id', None)).exists() else None)
    subject = test.subject
    semester = getattr(test, 'semester', None)
    old_test_qs = StudentTest.objects.filter(
        student=request.user,
        subject=subject,
        semester=semester,
        completed=True,
    ...
    if group:
        old_test_qs = old_test_qs.filter(group=group)
    old_test = old_test_qs.first()
    if old_test:
    ...
    if request.method == 'POST':
        ...
        stest = StudentTest.objects.create(
            student=request.user,
            test=test,
            group=group if group else request.user.group,  # fallback to student's group for M2M case
            subject=subject,
            semester=semester,
