...
# === AJAX: Fanga va (ixtiyoriy) semestrga qarab mavjud savollar soni ===
@login_required
@require_GET
def subject_question_info(request):
    login_redirect = login_check(request)
    if login_redirect:
        return login_redirect
    if not hasattr(request.user, 'role') or request.user.role != 'controller':
    ...
    if not subject_id:
        return JsonResponse({'error': 'subject param kerak'}, status=400)
    qs = Question.objects.filter(subject_id=subject_id)
    if semester_id:
        qs = qs.filter(semester_id=semester_id)
    return JsonResponse({'count': qs.count()})

# === AJAX: Tasodifiy savollarni ko'rsatish (preview) ===
@login_required
@require_GET
def random_question_preview(request):
    login_redirect = login_check(request)
    if login_redirect:
        return login_redirect
    if not hasattr(request.user, 'role') or request.user.role != 'controller':
    ...
    if limit > 20:  # xavfsizlik / yukni cheklash
        limit = 20
    if not subject_id:
        return JsonResponse({'error': 'subject param kerak'}, status=400)
    qs = Question.objects.filter(subject_id=subject_id)
    ...
    if total:
        ...
        _r.shuffle(ids)
        ids = ids[:limit]
        for q in Question.objects.filter(id__in=ids).select_related('subject'):
            sample.append({
                'id': q.id,
                'text': q.text[:180],
    ...
    return JsonResponse({'total': total, 'sample': sample})
...
def edit_test(request, test_id):
    ...
    # Talabalar allaqachon topshirganmi?
    existing_attempts = test.student_tests.exists()
    if request.method == 'POST':
        raw_subject = request.POST.get('subject')
        subject_id = None
        semester_id_for_test = None
        ...
        # Agar savollar qayta generatsiya qilinmasa va student attempt bor bo'lsa, question_count o'zgarishi ruxsat etilmaydi
        regenerate = request.POST.get('regenerate_questions') == '1'
        active_flag = request.POST.get('active') == '1'
        # active maydonini yangilash
        test.active = active_flag
        # updated_by ni yozamiz
        test.updated_by = request.user

        if existing_attempts and not regenerate:
            # question_count yoki total_score o'zgargan bo'lsa bloklaymiz
            if question_count != test.question_count or total_score != test.total_score:
                return render(request, 'controller_panel/edit_test.html', {
                    'groups': groups,
                    'kafedralar': kafedralar,
                    'bulimlar': bulimlar,
                    'subjects': subjects,
                    'test': test,
                    'error': 'Talabalar allaqachon testni bajargan. Savollarni qayta tuzmasangiz savollar soni yoki umumiy ballni o‘zgartira olmaysiz.',
        ...
        test.start_time = timezone.now()  # tahrir timestamp sifatida
        test.save()

        if regenerate:
            # Savollarni yangidan tuzamiz
            ...
            bulk = [TestQuestion(test=test, question=q, score=score_per_question) for q in selected_questions]
...
def add_test(request):
    ...
    if request.method == 'POST':
        ...
        video_file = request.FILES.get('video_file')
        required_missing = []
        for field_name, val in [('subject', subject_id), ('question_count', question_count), ('total_score', total_score), ('duration', duration_str)]:
            if not val:
        ...
        if duration is None:
            ...
            return render(request, 'controller_panel/add_test.html', context)
        subject = Subject.objects.get(id=subject_id)
        # Filter questions by subject + optional semester + chosen teachers
