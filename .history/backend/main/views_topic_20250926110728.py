...
def create_test(request, topic_id):
    ...
    test = TopicTest.objects.create(topic=topic, title=title, question_count=question_count, total_score=total_score, duration=timedelta(minutes=duration_minutes), created_by=request.user, question_ids=chosen)
...
def start_topic_test(request, test_id):
    ...
    # Group check
    if request.user.group_id != test.topic.group_id:
    ...
    if created or not st.randomized_question_ids:
        ...
        st.save()
    return render(request, 'topic_panel/test_run.html', {'test': test, 'student_test': st})
...
def submit_answer(request, student_test_id):
    ...
    ans.save()
    # update total
    agg = st.answers.aggregate(s=models.Sum('score'))
