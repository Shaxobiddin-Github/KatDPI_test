...
def topic_student_login(request):
    ...
    context = {}
    # Agar allaqachon login bo'lsa va POST emas bo'lsa to'g'ridan dashboard
    if request.user.is_authenticated and getattr(request.user, 'role', '') == 'student' and request.method != 'POST':
    ...
    if request.method == 'POST':
        ...
        if not first_name or not last_name or not group_id:
            ...
            try:
                ...
                # Tanlangan guruhdagi aniq moslik
                qs_exact = User.objects.filter(first_name__iexact=first_name,
                                               last_name__iexact=last_name,
                                               group=grp,
                                               role='student')
                if qs_exact.count() == 1:
                    login(request, qs_exact.first())
                    return redirect('topic:student_dashboard')
                # Boshqa guruh(lar)da shu ism-familiya bormi?
                qs_other = User.objects.filter(first_name__iexact=first_name,
                                               last_name__iexact=last_name,
                                               role='student').exclude(group=grp)
                if qs_other.exists():
                    context['error'] = 'Bu ism boshqa guruhga tegishli yoki tanlangan guruhda mavjud emas'
                else:
                    context['error'] = 'Siz tizimda mavjud emassiz'
        context.update({'first_name': first_name, 'last_name': last_name, 'group_selected': group_id})
    # Groups list (limit for dropdown)
    context['groups'] = Group.objects.all().order_by('name')[:500]
...
def topic_question_detail(request, question_id):
    ...
    data = {
        ...
        'question_type_display': q.get_question_type_display() if hasattr(q, 'get_question_type_display') else (
            'Bitta to‘g‘ri javob' if q.question_type in ('single','single_choice') else
            'Ko‘p to‘g‘ri javob' if q.question_type in ('multi','multiple','multiple_choice','multi_choice') else
            'Moslashtirish' if q.question_type in ('matching',) else
            'Bo‘sh joyni to‘ldirish' if q.question_type in ('fill_in_blank',) else
            'Jumlalarni tartiblash' if q.question_type in ('sentence_ordering',) else
            'To‘g‘ri/Yolg‘on' if q.question_type in ('true_false',) else q.question_type
        ),
...
def topic_student_personal_stats(request, topic_id):
    # Student only, show own runs per topic (optionally filtered by a specific test)
    ...
    if getattr(request.user, 'role', '') == 'student':
        ...
        if test_filter:
            try:
                runs = runs.filter(test_id=int(test_filter))
            except ValueError:
                pass
...
# ---------- Analytics: Student Personal Overall Stats ----------
@login_required
def topic_student_personal_overall(request):
    if getattr(request.user, 'role', '') != 'student':
        return HttpResponseForbidden('Faqat talaba')
    runs = TopicStudentTest.objects.filter(student=request.user, completed=True).select_related('test__topic').order_by('-finished_at')
    items = []
    for r in runs:
        total_q = len(r.test.question_ids)
        correct = r.answers.filter(is_correct=True).count()
        ...
        items.append({
            ...
            'topic_title': r.test.topic.title,
            'test_id': r.test.id,
            'test_title': r.test.title,
            'score': r.total_score,
            'percent': percent,
    ...
    if items:
        ...
        total_attempts=avg_percent=best_percent=avg_score=best_score=pass_rate=0
    personal_stats={
        'total_attempts':total_attempts,
        'avg_percent':avg_percent,
        'best_percent':best_percent,
...
def delete_topic(request, topic_id):
    """Delete an entire topic (and cascade its related objects) owned by the current teacher.
    Returns JSON {ok:True} or {ok:False,error:"..."}.
    """
    topic = get_object_or_404(Topic, id=topic_id, created_by=request.user)
    # (Optional) You could protect if there are attempts; for now allow direct delete.
    topic.delete()
    return JsonResponse({'ok': True})
