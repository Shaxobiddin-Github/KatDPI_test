...
def topic_question_detail(request, question_id):
    ...
    data = {
        ...
        'question_type_display': q.get_question_type_display() if hasattr(q, 'get_question_type_display') else (
            ...
            'Moslashtirish' if q.question_type == 'matching' else
            'Bo‘sh joyni to‘ldirish' if q.question_type == 'fill_in_blank' else
            'Jumlalarni tartiblash' if q.question_type == 'sentence_ordering' else
            'To‘g‘ri/Yolg‘on' if q.question_type == 'true_false' else q.question_type
        ),
