...
from django.http import JsonResponse
from main.models import IntroVideo

@login_required
def get_intro_video_controller(request):
    # Controller foydalanuvchi subject+semester uchun o'qituvchi yuklagan intro videoni ko'radi
    if not hasattr(request.user, 'role') or request.user.role != 'controller':
        return JsonResponse({'error': 'Ruxsat yo‘q'}, status=403)
    subject_id = request.GET.get('subject_id')
    semester_id = request.GET.get('semester_id') or None
    if not subject_id:
        return JsonResponse({'video_url': None, 'video_file': None})
    match_type = 'none'
    iv = None
    base = IntroVideo.objects.filter(subject_id=subject_id)
    # 1) Exact semester match
    if semester_id:
        iv = base.filter(semester_id=semester_id).order_by('-created_at').first()
        if iv:
            match_type = 'exact'
    # 2) Fallback to semesterless if not found
    if not iv:
        iv = base.filter(semester__isnull=True).order_by('-created_at').first()
        if iv and match_type == 'none':
            match_type = 'fallback_semesterless'
    # 3) As a last resort, any semester's latest
    if not iv:
        iv = base.order_by('-created_at').first()
        if iv and match_type == 'none':
            match_type = 'any'
    if not iv:
        return JsonResponse({'video_url': None, 'video_file': None, 'match': match_type})
    return JsonResponse({'video_url': iv.video_url, 'video_file': bool(iv.video_file), 'match': match_type})
...
def add_test(request):
    ...
    if request.method == 'POST':
        ...
        # Video maydonlari
        video_url = (request.POST.get('video_url') or '').strip()
        # YouTube URL bo'lsa, embed formatga aylantiramiz (nocookie + shorts qo'llab-quvvatlash)
        ...
        video_file = request.FILES.get('video_file')
        # Agar controller hech narsa kiritmagan bo'lsa, o'qituvchi intro videoni (file) yuklagan bo'lishi mumkin
        if (not video_url and not video_file) and subject_id:
            try:
                qs_iv = IntroVideo.objects.filter(subject_id=subject_id)
                if semester_id_for_test:
                    qs_iv = qs_iv.filter(semester_id=semester_id_for_test)
                else:
                    qs_iv = qs_iv.filter(semester__isnull=True)
                iv_obj = qs_iv.order_by('-created_at').first()
                if iv_obj:
                    if iv_obj.video_url and not video_url:
                        video_url = iv_obj.video_url
                    if iv_obj.video_file and not video_file:
                        # Faylni bevosita ko'chirish o'rniga modelga reference sifatida saqlash uchun Test ga yozamiz (FileField ni reuse qilish)
                        video_file = iv_obj.video_file
            except Exception:
                pass
