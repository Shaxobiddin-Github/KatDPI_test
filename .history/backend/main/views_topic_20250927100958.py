...
def topic_test_participants_stats(request, topic_id, test_id):
    ...
    per_question_option_selection = defaultdict(lambda: defaultdict(int))  # qid -> opt_id -> count
    ...
    for run in runs:
        ...
        for qmeta in question_meta:
            ...
            for opt_id in sel_ids:
                per_question_option_selection[qid][opt_id] += 1
    ...
    for qmeta in question_meta:
        ...
        # Build option selection counts list
        opt_counts = []
        for opt_id, letter in qmeta['option_sequence']:
            # find matching option dict in options_data and augment with selected_count for modal usage
            # (options_data built earlier without counts)
            # We'll defer setting on the option dict after computing count
            sel_count = per_question_option_selection[qid].get(opt_id, 0)
            # attach to corresponding option dict
            for od in qmeta['options_data']:
                if od['id'] == opt_id:
                    od['selected_count'] = sel_count
                    break
            opt_counts.append({
                'option_id': opt_id,
                'letter': letter,
        ...
        question_summaries.append({
            ...
            'unanswered': len(rows) - ans_cnt if rows else 0,
            'incorrect': ans_cnt - corr_cnt,
        })
