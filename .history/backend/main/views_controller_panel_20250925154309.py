...
def download_student_import_template(request):
    ...
    # Moslashuvchanlik uchun asosiy sarlavhalarni beramiz
    ws.append(['firstname', 'lastname', 'group', 'role'])
    # Namuna qatorlar
    ws.append(['Ali', 'Valiyev', 'CS-101', 'student'])
    ws.append(['Laylo', 'Karimova', 'CS-102', 'student'])
    output = io.BytesIO()
    wb.save(output)
    output.seek(0)
    response = HttpResponse(output, content_type='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
...
def add_user(request):
    ...
    if request.method == 'POST':
        ...
        if request.POST.get('import_type') == 'student' and request.FILES.get('excel_file'):
            ...
            idx_role = find_idx(['role', 'roli'])

            if idx_first is None or idx_last is None:
                request.session['import_result'] = {
                    'errors': ["Sarlavha topilmadi. Kerakli ustunlar: firstname, lastname (ixtiyoriy: group, role)"],
                }
                return redirect('add_user')
            ...
            with transaction.atomic():
                for row in ws.iter_rows(min_row=2, values_only=True):
                    ...
                    role = str(row[idx_role]).strip().lower() if idx_role is not None and row[idx_role] is not None else 'student'
                    if role == 'talaba':
                        role = 'student'

                    # Only process students for now
                    if role != 'student':
                        errors.append(f"{first} {last}: role '{role}' qo\'llab-quvvatlanmaydi (faqat student).")
                        continue
