...
def topic_question_detail(request, question_id):
    ...
    if getattr(request.user, 'role', '') == 'student':
        gid = request.user.group_id
        allowed = False
        # Prefer matching via student's randomized list (run may persist even if teacher edited test later)
        candidate_runs = TopicStudentTest.objects.filter(student=request.user, test__groups__id=gid)
        # Try to narrow by randomized_question_ids JSON contains if backend supports it; ignore errors silently
        try:
            from django.db.models import Q
            candidate_runs = candidate_runs.filter(
                Q(randomized_question_ids__contains=[q.id]) | Q(test__question_ids__contains=[q.id])
            )
        except Exception:
            # Fallback: keep unfiltered candidate_runs
            pass
        for run in candidate_runs.select_related('test'):
            # Accept if question originally in test.question_ids OR in stored randomized list (covers post-edit changes)
            if q.id in (run.randomized_question_ids or []) or q.id in run.test.question_ids:
                allowed = True
                logging.getLogger('api').info('QUESTION_ACCESS_OK user=%s q=%s test=%s rand_len=%s', request.user.id, q.id, run.test_id, len(run.randomized_question_ids or []))
                break
        if not allowed:
            logging.getLogger('api').warning('QUESTION_ACCESS_DENY user=%s q=%s candidate_runs=%s', request.user.id, q.id, list(candidate_runs.values_list('id', flat=True)))
    ...
    if getattr(request.user, 'role', '') == 'student':
        # Prefer run where randomized list contains this question (more accurate)
        active_run = TopicStudentTest.objects.filter(student=request.user, randomized_question_ids__contains=[q.id]).order_by('-started_at').first()
        if not active_run:
...
def topic_test_participants_stats(request, topic_id, test_id):
    topic = get_object_or_404(Topic, id=topic_id, created_by=request.user)
    test = get_object_or_404(TopicTest, id=test_id, topic=topic)
    runs = TopicStudentTest.objects.filter(test=test, completed=True).select_related('student').prefetch_related('answers__question')
    rows = []
    for run in runs:
        answers = run.answers.all()
        correct = answers.filter(is_correct=True).count()
        incorrect = answers.filter(is_correct=False).count()
        total_q = len(test.question_ids) or (correct + incorrect)
        percent = round((correct/total_q)*100, 1) if total_q else 0
        rows.append({
            'student': run.student,
