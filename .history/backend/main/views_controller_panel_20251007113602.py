...
def edit_test(request, test_id):
    ...
    if request.method == 'POST':
        ...
        # Faol holatni o'zgartirish (checkbox bo'lmasa eski qiymat saqlanadi)
        if 'active' in request.POST:
            test.active = True if request.POST.get('active') == '1' else False
        # Attemptlar mavjud bo'lsa va regenerate belgilanmagan bo'lsa question_count / total_score ni o'zgartirishga ruxsat bermaymiz
        if existing_attempts and not regenerate:
            # foydalanuvchi boshqa qiymat yuborgan bo'lsa — e'tiborsiz qoldiramiz (shablonda ogohlantirish bo'ladi)
            pass
        else:
        ...
        if regenerate:
            ...
            selected_questions = questions.order_by('?')[:test.question_count]
            score_per_question = test.total_score / test.question_count if test.question_count else 0
            from main.models import TestQuestion as TQ
            TQ.objects.bulk_create([TQ(test=test, question=q, score=score_per_question) for q in selected_questions])
