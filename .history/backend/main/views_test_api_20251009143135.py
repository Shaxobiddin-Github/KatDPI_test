...
def subject_groups_for_results(request, subject_name):
    ...
    qs = StudentTest.objects.filter(completed=True, test__subject__name=subject_name).values_list('test__group__name', flat=True)
    # Include M2M groups participation by student group membership
    # Fall back to distinct names excluding None/''
    names = sorted({name for name in qs if name})
