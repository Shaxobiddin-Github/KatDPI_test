...
from main.models import Subject, Group, Faculty, University, Question, AnswerOption, Semester, GroupSubject, IntroVideo
...
def teacher_dashboard(request):
    ...
    # Intro videolarni xaritasi (subject_id, semester_id)->IntroVideo
    intro_map = {}
    for iv in IntroVideo.objects.filter(created_by=request.user, subject_id__in=[k[0] for k in grouped.keys()]):
        key = (iv.subject_id, iv.semester_id)
        intro_map[key] = iv
    return render(request, 'teacher_panel/dashboard.html', {
        'subject_semesters': subject_semesters,
        'intro_videos': intro_map,
    })

@login_required
def get_intro_video(request):
    if not hasattr(request.user, 'role') or request.user.role != 'teacher':
        return JsonResponse({'error': 'Ruxsat yo‘q'}, status=403)
    subject_id = request.GET.get('subject_id')
    semester_id = request.GET.get('semester_id') or None
    q = IntroVideo.objects.filter(created_by=request.user, subject_id=subject_id)
    if semester_id:
        q = q.filter(semester_id=semester_id)
    else:
        q = q.filter(semester__isnull=True)
    iv = q.first()
    if not iv:
...
def save_intro_video(request):
    ...
    if not subject_id:
        return JsonResponse({'error': 'Subject majburiy'}, status=400)
    if not (video_url or video_file):
        return JsonResponse({'error': 'Kamida URL yoki fayl kerak'}, status=400)
    iv_qs = IntroVideo.objects.filter(created_by=request.user, subject_id=subject_id)
    if semester_id:
        iv_qs = iv_qs.filter(semester_id=semester_id)
    else:
        iv_qs = iv_qs.filter(semester__isnull=True)
    iv = iv_qs.first()
    if not iv:
        iv = IntroVideo(created_by=request.user, subject_id=subject_id, semester_id=semester_id)
    # Replace existing
    if video_url:
        iv.video_url = video_url
    if video_file:
        if iv.video_file:
            iv.video_file.delete(save=False)
...
def add_question(request):
    ...
    if request.method == 'POST':
        ...
        # If student target with semester selected, attach semester to question
        if target == 'student':
            semester_id = request.POST.get('semester')
            if semester_id:
                q.semester_id = int(semester_id)
        ...
        context['success'] = "Savol muvaffaqiyatli qo'shildi"
        return render(request, 'teacher_panel/add_question.html', context)
    return render(request, 'teacher_panel/add_question.html', context)
