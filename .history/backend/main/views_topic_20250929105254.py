...
def topic_student_personal_stats(request, topic_id):
    ...
    if getattr(request.user, 'role', '') == 'student':
        ...
        # Overall stats JSON for frontend (selection switching)
        personal_stats_js = mark_safe(json.dumps(personal_stats, default=str))
        # Distinct tests list (latest first as in runs ordering)
        tests_select = []
        seen_tests = set()
        for r in runs:
            if r.test_id not in seen_tests:
                seen_tests.add(r.test_id)
                tests_select.append({'id': r.test.id, 'title': r.test.title})
        default_test_id = items[0]['test_id'] if items else None
        return render(request, 'topic_panel/analytics_student_personal.html', {
            'topic': topic,
            'items': items,
            'items_js': items_js,
            'personal_stats_js': personal_stats_js,
            'tests_select': tests_select,
