...
def testapi_test(request, test_id):
    ...
    # Agar test uchun video mavjud bo'lsa, avval videoni ko'rsatamiz
    if (getattr(test, 'video_url', None) or getattr(test, 'video_file', None)) and not request.session.get(f'video_seen_{test.id}'):
        if request.method == 'POST' and request.POST.get('ack_video') == '1':
            request.session[f'video_seen_{test.id}'] = True
    ...
    if request.method == 'POST':
        ...
        if effective_group is None:
            ...
            try:
                ...
                pass
        # Duplicate guard: Agar shu test uchun allaqachon completed va can_retake False bo'lsa – qayta yaratmaymiz
        existing_final = StudentTest.objects.filter(student=request.user, test=test, completed=True, can_retake=False).first()
        if existing_final:
            return redirect('testapi_result', existing_final.id)
        # Agar incomplete yozuv bo'lsa – o'shani ishlatamiz (session qayta POST kelganda)
        stest = StudentTest.objects.filter(student=request.user, test=test, completed=False).order_by('-start_time').first()
        if not stest:
            # get_or_create bilan yaratilishini kafolatlash (unique constraint group/subject/semester asosida emas – retake holati uchun can_retake ko'rib chiqildi)
