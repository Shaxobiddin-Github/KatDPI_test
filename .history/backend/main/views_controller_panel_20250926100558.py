...
def get_intro_video_controller(request):
    ...
    qs = IntroVideo.objects.filter(subject_id=subject_id)
    if semester_id:
        qs = qs.filter(semester_id=semester_id)
    else:
        qs = qs.filter(semester__isnull=True)
    # Controller bir nechta o'qituvchining videolaridan bittasini tanlashi uchun hozircha birinchi
    iv = qs.order_by('-created_at').first()
    if not iv:
        return JsonResponse({'video_url': None, 'video_file': None})
