...
def subject_groups_for_results(request, subject_name):
    ...
    st_qs = (
        StudentTest.objects.filter(completed=True, test__subject__name=subject_name)
        .select_related('test', 'test__group', 'student', 'student__group')
        .prefetch_related('test__groups')
    )
    names_set = set()
    for st in st_qs:
        group_obj = None
        try:
            if getattr(st.test, 'group', None):
                group_obj = st.test.group
                ...
                if group_obj is None:
                    try:
                        first = st.test.groups.all().first()
                    except Exception:
                        first = None
                    if first:
                        group_obj = first
            ...
            group_obj = None
        if group_obj and getattr(group_obj, 'name', None):
            names_set.add(group_obj.name)
    names = sorted(names_set)
    return JsonResponse({ 'groups': names })
    ...
    for stest in student_tests:
        ...
        if answers.exists():
            for answer in answers:
                ...
                # Build names from multiple sources, then de-dup and drop empty
                names = set()
