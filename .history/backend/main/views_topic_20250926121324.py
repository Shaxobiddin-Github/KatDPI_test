...
def topic_student_dashboard(request):
    ...
    return render(request, 'topic_panel/student_dashboard.html', {'tests': enriched})
...
def topic_question_detail(request, question_id):
    ...
    if getattr(request.user, 'role', '') == 'student':
        # Student allowed if belongs to any test where question's test includes their group
        # Simpler: allow if student's group equals topic.group OR exists any TopicTest with this question and student's group in groups
        gid = request.user.group_id
        if gid != q.topic.group_id:
            has_access = TopicTest.objects.filter(topic=q.topic, groups__id=gid, question_ids__contains=[q.id]).exists()
            if not has_access:
