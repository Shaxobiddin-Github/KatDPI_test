...
class IntroVideo(models.Model):
    subject = models.ForeignKey('Subject', on_delete=models.CASCADE, related_name='intro_videos')
    semester = models.ForeignKey('Semester', on_delete=models.CASCADE, null=True, blank=True, related_name='intro_videos')
    video_url = models.URLField(max_length=500, blank=True, null=True)
    video_file = models.FileField(upload_to='intro_videos/', blank=True, null=True)
    created_by = models.ForeignKey(User, on_delete=models.CASCADE, related_name='intro_videos')
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)

    class Meta:
        constraints = [
            models.UniqueConstraint(fields=['subject', 'semester', 'created_by'], name='uniq_intro_video_per_teacher_subj_sem')
        ]
    ...
    def clean(self):
        ...
        if not self.video_url and not self.video_file:
            raise ValidationError('Kamida bitta: video URL yoki fayl kerak')

    def __str__(self):
        sem = f" / Sem {self.semester.number}" if self.semester else ''
        return f"IntroVideo {self.subject.name}{sem} ({self.created_by.username})"
