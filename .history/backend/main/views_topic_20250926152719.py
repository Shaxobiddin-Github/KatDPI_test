...
def create_topic(request):
    ...
    try:
        ...
        return JsonResponse({'error': 'Group ID noto\'g\'ri'}, status=400)

    subject = Subject.objects.filter(id=subject_id_int).first()
    if not subject:
        return JsonResponse({'error': 'Subject topilmadi'}, status=400)
    group = Group.objects.filter(id=group_id_int).first()
    if not group:
        return JsonResponse({'error': 'Group topilmadi'}, status=400)
