...
def edit_test(request, test_id):
    ...
    if not (request.user.is_superuser or request.user.is_staff):
        ...
        if has_participation:
            return render(request, 'controller_panel/edit_test.html', {
                ...
                'error': "Bu testda talabalar allaqachon qatnashgan. Tahrirlash taqiqlangan." 
            })
    ...
    if request.method == 'POST':
        ...
        # Faqat:
        #  * remove_video=1 bo'lsa tozalanadi;
        #  * yangi video_url kiritilsa URL yangilanadi (embed shaklida);
        #  * yangi video_file yuborilsa fayl yangilanadi.
        # Bo'sh ("") URL maydoni endi mavjud URL'ni o'chirmaydi (avvalgi xulqdan farq qiladi).
        if request.POST.get('remove_video') == '1':
            # Foydalanuvchi ataylab o'chirmoqda
            test.video_url = None
            if getattr(test, 'video_file', None):
                test.video_file.delete(save=False)
            test.video_file = None
        else:
            # Faqat foydalanuvchi yangi qiymat bersa yoki fayl yuborsa o'zgartiramiz
            if video_url:  # kiritilgan (bo'sh emas) URL
                test.video_url = video_url
            if video_file:  # yangi fayl yuklangan
        ...
        # Filter questions by context: semester for student-type, kafedra/bulim for others
        questions = Question.objects.filter(subject=subject)
        if getattr(test, 'semester_id', None):
            questions = questions.filter(semester_id=test.semester_id)
        if getattr(test, 'kafedra_id', None):
            questions = questions.filter(kafedra_id=test.kafedra_id)
        if getattr(test, 'bulim_id', None):
            questions = questions.filter(bulim_id=test.bulim_id)
...
def delete_test(request, test_id):
    ...
    if request.method == 'POST':
        if request.user.is_superuser or request.user.is_staff:
            ...
            # Oddiy controller: faqat qatnashuv yo'q bo'lsa o'chiradi
            has_participation = StudentTest.objects.filter(test_id=test.id).exists()
            if has_participation:
                messages.error(request, "Talabalar qatnashgan testni o'chirib bo'lmaydi.")
            else:
                Test.objects.filter(id=test.id).delete()
    return redirect('controller_dashboard')
...
def controller_dashboard(request):
    ...
    if not hasattr(request.user, 'role') or request.user.role != 'controller':
        return redirect('/api/login/')
    tests = Test.objects.filter(created_by=request.user).prefetch_related('groups','subject','semester')
    now = timezone.now()
    test_list = []
    for test in tests:
        status = "Faol"
...
def extend_test_time(request, test_id):
    ...
    if request.method == 'POST':
        try:
            ...
            return render(request, 'controller_panel/extend_time.html', {'test': test, 'error': 'Noto\'g\'ri raqam'})
        reactivate = request.POST.get('reactivate') == '1'
        if add_minutes > 0:
            test.duration += timedelta(minutes=add_minutes)
        if reactivate:
            test.start_time = timezone.now()
            test.active = True
        test.updated_by = request.user
        test.save()
        return redirect('controller_dashboard')
    return render(request, 'controller_panel/extend_time.html', {'test': test})
...
def add_test(request):
    ...
    if not hasattr(request.user, 'role') or request.user.role != 'controller':
        return redirect('/api/login/')
    # New flow: no group selection at creation; controller chooses subject + semester + teacher(s)
    target = request.GET.get('target') or request.POST.get('target') or 'student'
    context = {
        'target': target,
        'subjects': Subject.objects.all(),
        'semesters': Semester.objects.all(),  # only used for student
        'kafedralar': Kafedra.objects.all(),   # tutor
        'bulimlar': Bulim.objects.all(),       # employee
    }
    if request.method == 'POST':
        raw_subject = request.POST.get('subject')
        subject_id = raw_subject
        semester_id_for_test = None
        kafedra_id_for_test = None
        bulim_id_for_test = None
        if target == 'student':
            semester_id_for_test = request.POST.get('semester') or None
        elif target == 'tutor':
            kafedra_id_for_test = request.POST.get('kafedra') or None
        elif target == 'employee':
            bulim_id_for_test = request.POST.get('bulim') or None
        teacher_ids = request.POST.getlist('teacher_ids')  # multi-select
        ...
        test_title = request.POST.get('title') or 'Test'
        video_url = (request.POST.get('video_url') or '').strip()
        def _to_youtube_embed(url: str) -> str:
            try:
                ...
                if 'youtube.com/shorts/' in u:
                    vid = u.split('youtube.com/shorts/', 1)[1].split('?')[0].strip('/')
                ...
                return url
        video_url = _to_youtube_embed(video_url)
        video_file = request.FILES.get('video_file')
        required_missing = []
        required_base = [('subject', subject_id), ('question_count', question_count), ('total_score', total_score), ('duration', duration_str)]
        if target == 'student':
            # semester ixtiyoriy deyilgan – shart qo'shmaymiz
            pass
        elif target == 'tutor':
            required_base.append(('kafedra', kafedra_id_for_test))
        elif target == 'employee':
            required_base.append(('bulim', bulim_id_for_test))
        for field_name, val in required_base:
            if not val:
                required_missing.append(field_name)
        if required_missing:
            context['error'] = f"Majburiy maydonlar yetishmaydi: {', '.join(required_missing)}"
        ...
        # Distinct to be safe
        qfilter = qfilter.distinct()
        if qfilter.count() < question_count:
        ...
        test = Test.objects.create(
            ...
            kafedra_id=kafedra_id_for_test,
            bulim_id=bulim_id_for_test,
        ...
        # Random selection
        selected_questions = qfilter.order_by('?')[:question_count]
        score_per_question = total_score / question_count
        for question in selected_questions:
            TestQuestion.objects.create(test=test, question=question, score=score_per_question)
        return redirect('controller_dashboard')
    return render(request, 'controller_panel/add_test.html', context)
# AJAX: O'qituvchilarni fan va semestr bo'yicha filtrlaydigan endpoint
@login_required
@require_GET
def get_teachers_by_subject_semester(request):
    if not hasattr(request.user, 'role') or request.user.role != 'controller':
    ...
    # Filter by container stored on Question itself
    if kafedra_id:
...
def assign_test(request, test_id):
    ...
    if request.method == 'POST':
        ids = request.POST.getlist('group_ids')
        # Filter only valid ints
        valid_ids = [g.id for g in all_groups if str(g.id) in ids]
        # Conflict check: shu subject + semester + group kombinatsiya boshqa testda bormi?
...
@login_required
def assign_test_kafedra(request, test_id):
    login_redirect = login_check(request)
    if login_redirect:
        return login_redirect
    if not hasattr(request.user, 'role') or request.user.role != 'controller':
    ...
    all_kaf = Kafedra.objects.all().order_by('name')
    if request.method == 'POST':
        # Active toggle
        test.active = bool(request.POST.get('active'))
        test.save(update_fields=['active'])
        ids = request.POST.getlist('kafedra_ids')
        valid_ids = [k.id for k in all_kaf if str(k.id) in ids]
        test.kafedralar.set(valid_ids)
        return redirect('controller_dashboard')
    assigned_ids = set(test.kafedralar.values_list('id', flat=True))
