...
def add_test(request):
    ...
    if request.method == 'POST':
        ...
        bulim = None
        if target == 'student':
            # endi faqat bitta guruh: field name "groups" (single select)
            single_gid = request.POST.get('groups') or ''
            group_ids = [single_gid] if single_gid else []
            if not group_ids:
                context['error'] = "Iltimos, guruhni tanlang!"
                return render(request, 'controller_panel/add_test.html', context)
        elif target == 'tutor':
            kafedra_id = request.POST.get('kafedra')
            kafedra = Kafedra.objects.get(id=kafedra_id)
