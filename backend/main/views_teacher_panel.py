...
def add_question(request):
    ...
    if target == 'student':
        ...
        # POST (bir nechta guruh) kelganda fanlarni yo'qotmaslik uchun avval POST dan guruhni aniqlaymiz
        post_group_ids = []
        if request.method == 'POST':
            post_group_ids = request.POST.getlist('groups') or []
            if not post_group_ids:
                raw = request.POST.get('groups')
                if raw:
                    post_group_ids = [g for g in raw.split(',') if g]
        group_id = request.GET.get('group') or (post_group_ids[0] if post_group_ids else request.POST.get('group'))
    ...
    if request.method == 'POST':
        ...
        if target == 'student':
            ...
            context['selected'].update({'faculty': faculty_id, 'group': first_group, 'group_ids': group_ids, 'semester': semester_id})
        ...
        # Yangi savol qo'shilgandan keyin ham fanlar selecti to'lib turishi uchun qaytadan filterla
        if target == 'student' and group_id and semester_id:
            from main.models import GroupSubject
            group_subjects = GroupSubject.objects.filter(group_id=group_id, semester_id=semester_id)
            context['subjects'] = Subject.objects.filter(id__in=group_subjects.values_list('subject_id', flat=True))
        return render(request, 'teacher_panel/add_question.html', context)
